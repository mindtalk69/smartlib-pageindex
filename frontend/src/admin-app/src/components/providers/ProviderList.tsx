...
function getHealthStatusBadge(status: string | null) {
  switch (status) {
    case 'healthy':
      return { variant: 'outline' as const, icon: CheckCircle, color: 'text-green-600 dark:text-green-400' }
    case 'degraded':
      return { variant: 'secondary' as const, icon: AlertTriangle, color: 'text-yellow-600 dark:text-yellow-400' }
    case 'offline':
      return { variant: 'destructive' as const, icon: XCircle, color: 'text-red-600 dark:text-red-400' }
    default:
      return { variant: 'secondary' as const, icon: HelpCircle, color: 'text-gray-500 dark:text-gray-400' }
  }
}
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-[180px]">Last Health Check</TableHead>
            <TableHead className="w-[120px]">Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {providers.map((provider) => {
            const HealthIcon = getHealthStatusBadge(provider.health_status).icon
            return (
              <TableRow key={provider.id}>
                ...
                <TableCell>
                  <Badge
                    variant={getHealthStatusBadge(provider.health_status).variant}
                    className={`flex items-center gap-1 w-fit ${getHealthStatusBadge(provider.health_status).color}`}
                  >
