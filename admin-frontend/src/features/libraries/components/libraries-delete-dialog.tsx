...
type LibrariesDeleteDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  currentRow: Library
}

export function LibrariesDeleteDialog({
  open,
  onOpenChange,
  currentRow,
}: LibrariesDeleteDialogProps) {
  ...
  const handleDelete = async () => {
    ...
    setIsLoading(true)
    const success = await deleteLibrary(currentRow.id)
    setIsLoading(false)

    if (success) {
      toast.success(`Library "${currentRow.name}" has been deleted`)
      onOpenChange(false)
    } else {
