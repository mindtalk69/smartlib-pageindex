...
type CatalogsDeleteDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  currentRow: Catalog
}

export function CatalogsDeleteDialog({
  open,
  onOpenChange,
  currentRow,
}: CatalogsDeleteDialogProps) {
  ...
  const handleDelete = async () => {
    ...
    setIsLoading(true)
    const success = await deleteCatalog(currentRow.id)
    setIsLoading(false)

    if (success) {
      toast.success(`Catalog "${currentRow.name}" has been deleted`)
      onOpenChange(false)
    } else {
