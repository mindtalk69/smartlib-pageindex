...
type ProvidersDeleteDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  currentRow: Provider
}

export function ProvidersDeleteDialog({
  open,
  onOpenChange,
  currentRow,
}: ProvidersDeleteDialogProps) {
  ...
  const handleDelete = async () => {
    ...
    setIsLoading(true)
    const success = await deleteProvider(currentRow.id)
    setIsLoading(false)

    if (success) {
      toast.success(`Provider "${currentRow.name}" has been deleted`)
      onOpenChange(false)
    } else {
