...
from sqlmodel import SQLModel, Field, Relationship, Column, JSON, String, Text, DateTime, Boolean, Integer, Float, ForeignKey, Table
...
class LLMPrompt(SQLModel, table=True):
    ...
    description: Optional[str] = Field(default=None, sa_column=Column(String(256), nullable=True))
...
class User(SQLModel, table=True):
    ...
    azure_oid: Optional[str] = Field(default=None, sa_column=Column(String, unique=True, nullable=True))
...
knowledge_groups_association = Table(
    ...
    SQLModel.metadata,
    Column('knowledge_id', Integer, ForeignKey('knowledges.id', ondelete='CASCADE'), primary_key=True),
...
class Group(SQLModel, table=True):
    ...
    knowledges: List["Knowledge"] = Relationship(back_populates="groups", sa_relationship_kwargs={"secondary": knowledge_groups_association})
...
class UserGroup(SQLModel, table=True):
    ...
    joined_at: datetime = Field(default_factory=datetime.utcnow, sa_column=Column(DateTime(timezone=True), server_default=func.now()))
...
class Knowledge(SQLModel, table=True):
    ...
    groups: List["Group"] = Relationship(back_populates="knowledges", sa_relationship_kwargs={"secondary": knowledge_groups_association})
...
class UploadedFile(SQLModel, table=True):
    ...
    file_size: int = Field()
    upload_time: datetime = Field(default_factory=datetime.utcnow, sa_column=Column(DateTime(timezone=True), server_default=func.now()))
...
class UrlDownload(SQLModel, table=True):
    ...
    is_ocr: bool = Field(default=False)
    is_az_doci: bool = Field(default=False)
    processed_at: datetime = Field(default_factory=datetime.utcnow, sa_column=Column(DateTime(timezone=True), server_default=func.now()))
...
class LibraryReference(SQLModel, table=True):
    ...
    added_at: datetime = Field(default_factory=datetime.utcnow, sa_column=Column(DateTime(timezone=True), server_default=func.now()))
...
class VectorReference(SQLModel, table=True):
    ...
    url_download_id: Optional[int] = Field(default=None, foreign_key="url_downloads.download_id")
...
class Document(SQLModel, table=True):
    ...
    brand_manufacturer_organization: Optional[str] = Field(default=None, sa_column=Column(String(255)))
    product_model_name_service: Optional[str] = Field(default=None, sa_column=Column(String(255)))
    created_at: datetime = Field(default_factory=datetime.utcnow, sa_column=Column(DateTime(timezone=True), server_default=func.now()))
