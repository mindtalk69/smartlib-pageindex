...
docker rmi smartlib-app:ultra-minimal smartlib-app:cpu-latest 2>/dev/null || true
...
echo "📊 Image Analysis:"
echo "─────────────────────────────────────────────────────────"
docker images smartlib-app:ultra-minimal --format "table {{.Repository}}\t{{.Tag}}\t{{.Size}}"

SIZE=$(docker images smartlib-app:ultra-minimal --format "{{.Size}}")
echo ""
echo "🎯 Target for Azure B1: <800MB"
echo "📏 Current size: $SIZE"

# Tag as cpu-latest for deployment
docker tag smartlib-app:ultra-minimal smartlib-app:cpu-latest
